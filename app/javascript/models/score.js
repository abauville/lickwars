...
export class Score {
    ...
    allMeasureStaveNotes.forEach((thisMeasureStaveNotes, index) => {
      if (index === 0) {
        ...
        stave = new VF.Stave(
          x + 10,
          40,
          measure_width + key_time_signature_width
        );
      ...
      const formatter = new VF.Formatter({ softmaxFactor: 20 })
        .joinVoices([voice])
        .format([voice], measure_width);
  ...
  addActionToNotes() {
    ...
    notes.forEach((note) => {
      note.setAttribute("data-action", "click->score#clickNote");
    });
  }
